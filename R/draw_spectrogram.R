...
#' @param frequency_range vector with the range of frequencies to be displayed for the spectrogram up to a maximum of \code{fs}/2. This is set to 0-5 kHz by default.
...
draw_spectrogram <- function (sound,
  ...
  if(!is.null(raven_annotation)){
    ...
    if("time_start" %in% names(raven_annotation) &
      ...
      warning("raven_annotation should have time_start, time_end, freq_low and freq_high columns")
    }
