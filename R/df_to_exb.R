#' df_to_exb
#'
#' Takes a dataframe and turns it into a .exb file
#'
#' @author Valeria Buntiakova <valleriabun@gmail.com>
#'
#' @param df dataframe that must include columns tier, id, content, tier_name, time_start, time_end and may include columns tier_type, tier_category, tier_speaker
#' @param name transcription name
#' @param outputPath path
#' @param referenced_file name of audio file 
#' @param ud_meta vector c(parameter:value) of metadata
#' @param speaker_table dataframe with speaker data; must include column id with speaker id; may include columns abbreviation, sex and any amount of feature columns (e.g. name, surname)
#'
#' @return file with transcription
#'
#' @examples
#' df_to_exb(Beckhams, 
#'           name='Beckhams', 
...
#'           ud_meta = c('language' = 'eng', 'place' = 'New York'),
#'           speaker_table=data.frame(id = c('SPK0', 'SPK1', 'SPK2'), 
#'                                    abbreviation = c('PAR', 'VIC', 'DAV'), 
#'                                    sex = c('m', 'f', 'f'),
#'                                    name = c('Michael', 'Victoria', 'David'), 
#'                                    surname = c('Parkinson', 'Beckham', 'Beckham')))
#' 
#' @export
#'
