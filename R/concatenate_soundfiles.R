#' Concatenate sounds
#'
#' Creates a sound file and a Praat TextGrid whose interval labels are the
#' original names of the sound
#'
#' @author George Moroz <agricolamz@gmail.com>
#'
#' @param path path to the directory with soundfiles.
#' @param result_file_name name of the result and annotation files.
#' @param annotation there are several variants: "textgrid" for Praat TextGrid,
#' "eaf" for ELAN's .eaf file, or "exb" for EXMARaLDA's .exb file
#' @return
#'
#' @examples
...
concatenate_soundfiles <- function(path,
  ...
  if (annotation == "textgrid") {
    ...
    annotation != "TextGrid" &
    !is.null(annotation)) {
    warning('Annotation was not created. The only variant for annotation
            argument are: "textgrid", "eaf", "exb" or NULL')
  }
}
