...
## Read linguistic file types into R
...
```{r}
flextext_to_df("zilo_test.flextext")
```

There is also an additional function for working with the `.flextext` format that convert it to a plain `docx` or `.html` format:

```{r, eval = FALSE}
create_glossed_text(flextext = "zilo_test.flextext", 
                    output_dir = getwd()) # you need to specify the path to the output folder
```

Here are some examples of the result files: [`.docx`](https://github.com/agricolamz/phonfieldwork/raw/master/docs/glossed_document.docx), [`.html`](https://agricolamz.github.io/phonfieldwork/glossed_document.html).
