...
For each speaker `s1` and `s2` there is a folder that containes three audiofiles. It is possible to check the length of each file:

```{r}
get_sound_duration(sounds_from_folder = "s1")
```
...
* file `.eaf` from ELAN  (just change the `system.file()` function to path to the file)
```{r}
...
```{r}
audacity_to_df(system.file("extdata", "test.txt", package = "phonfieldwork"))
```
