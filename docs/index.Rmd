...
For now `phonfieldwork` can work only with `.wav` files, but I will try to implement more possibilities in the future.

## Merge all data together

After all files are renamed, it is possible to merge into one:

```{r}
concatenate_soundfiles(filename = "s1_all",
                       path = "s1/")
```

This comand creates a new soundfile and an asociated Praat TextGrid:
