...
There is also an additional function for working with the `.flextext` format that convert it to a plain `.html` or `docx` format:

```{r, eval = FALSE}
create_glossed_text(flextext = "your_file.flextext", output_dir = getwd())
```
