...
It is also possible to analyze the whole folder using the `read_from_folder()` function. The first argument is the path to the folder. The second argument is the function name (without quotation marks) that should get information:


```r
read_from_folder(path = "s2/", get_sound_duration)
```
...
It is not kind of task that could accure within phonfieldwork phylosophy, but it also possible to merge multiple `.TextGrid`s with the same tier structure using `concatente_textgrids()` function.
...
```r
create_empty_textgrid(get_sound_duration("s2/s2_tip_1.wav"),
                      tiers = c("a", "b"),
                      path = "s2",
...
```
## Error in tuneR::readWave(file_name): File 's1/s1_sounds/2_s1_ı.wav' does not exist.
```
...
It is also possible to convert to LaTeX examples format using the `example_pkg` argument (possible values are: `gb4e`, `langsci`, `expex`, `philex`). There is also an additional [text](https://agricolamz.github.io/phonfieldwork/articles/manipulating_with_flextext_fo_df.html) about manipulation with `flextext_to_df()` output.
...
All those functions (`tier_to_df()`, `textgrid_to_df()`, `eaf_to_df()`, `exb_to_df()`, `audacity_to_df()`, `srt_to_df()`) except `flextext_to_df()` can be used within the `read_from_folder()` funtion in order to read multiple files of a certain type from some folder. The first argument is the path to the folder. The second argument is the function name (without quotation marks) that should get information.
