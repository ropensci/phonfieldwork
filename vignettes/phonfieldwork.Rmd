...
In case you want to create an empty TextGrid it is possible to use a `create_empty_textgrid()` function that takes some soundfile as a refence and creates


```r
create_empty_textgrid("s2/s2_tip_1.wav", tiers = c("a", "b"))
```
...
```
## Warning in if (!(output_format %in% c("html", "docx"))) {: the condition has length
